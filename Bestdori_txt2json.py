...
class ConfigManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "patterns": {
                # 通用模式，不再区分是否有引号，因为所有引号移除操作都推迟到最后处理
        ...
        try:
            with open(self.config_path, 'r', encoding='utf-8') as f:
                ...
                if "quote_categories" not in loaded_config.get("quotes", {}):
                    ...
                    self._save_config(loaded_config) # 保存更新后的配置
    ...
    def parse(self, line: str) -> Optional[Tuple[str, str]]:
        ...
        if match:
            try:
                ...
                logger.error(f"正则表达式 '{self.pattern.pattern}' 中缺少捕获组。请检查 config.yaml。")
                return None
        return None
...
class QuoteHandler:
    ...
    def is_quoted_by(self, text: str, active_quote_pairs: Dict[str, str]) -> bool:
        """检查文本是否被一组特定的、激活的引号包围"""
        stripped = text.strip()
        if len(stripped) < 2:
            return False
        ...
        return last_char == expected_closing

    def remove_quotes(self, text: str, active_quote_pairs: Dict[str, str]) -> str:
        """如果文本被指定的引号包围，则移除它们"""
        if self.is_quoted_by(text, active_quote_pairs):
            return text.strip()[1:-1].strip()
        return text    


class NarratorParser:
    """旁白解析器，现在主要用于最后阶段的引号移除"""
    
    def __init__(self, quotes_config: Dict[str, Any]):
        self.opening_quotes = set(quotes_config.get("opening_quotes", []))
        self.quote_pairs = quotes_config.get("quote_pairs", {})
    
    def is_quoted(self, text: str) -> bool:
        """检查文本是否被一对完整的、已知的引号包围"""
        stripped = text.strip()
        if len(stripped) < 2:
            return False
        
        first_char = stripped[0]
        if first_char not in self.opening_quotes:
            return False
        
        last_char = stripped[-1]
        expected_closing = self.quote_pairs.get(first_char)
        
        return last_char == expected_closing

    def remove_quotes(self, text: str) -> str:
        """如果文本被引号包围，则移除它们"""
        if self.is_quoted(text):
            return text.strip()[1:-1].strip()
        return text
...
class TextConverter:
    ...
    def _init_parsers(self):
        """初始化解析器"""
        speaker_pattern = self.patterns.get("speaker_pattern")
        if not speaker_pattern:
            logger.warning("在 config.yaml 中未找到 'speaker_pattern'。将使用安全的默认值。")
            speaker_pattern = r'^([\w\s]+)\s*[：:]\s*(.*)$'
    ...
    def convert_text_to_json_format(self, input_text: str, narrator_name: str = None, selected_quote_pairs: Optional[Dict[str, str]] = None) -> str:
        ...
        if selected_quote_pairs is None:
        ...
        def finalize_current_action():
            if current_action_body_lines:
                ...
                if finalized_body and selected_quote_pairs:
                    finalized_body = self.quote_handler.remove_quotes(finalized_body, selected_quote_pairs)
        ...
        for line in input_text.split('\n'):
            ...
            if parse_result:
                ...
                if speaker != current_action_name:
                    finalize_current_action()
                    current_action_body_lines = []
    ...
    def fuzzy_match_character(self, name: str) -> Optional[List[int]]:
        if name in self.character_mapping:
            return self.character_mapping[name]
        
        for char_name, char_ids in self.character_mapping.items():
...
class ModernConverterGUI:
    ...
    def setup_gui(self):
        """设置GUI界面"""
        self.root = tk.Tk()
        self.root.title("文本转JSON转换器 v2.5 (分类引号移除)")
        ...
        quote_frame = ttk.LabelFrame(main_frame, text="引号处理选项 (可多选)", padding="5")
        quote_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
        ...
        for category_name in quote_categories.keys():
            ...
            chk = ttk.Checkbutton(quote_frame, text=category_name, variable=var)
            chk.grid(row=0, column=col_count, sticky="w", padx=10)
        ...
        self.status_var = tk.StringVar(value="就绪 - 支持多种引号自动移除")
        ...
        for category_name, var in self.quote_category_vars.items():
            if var.get():  # 如果复选框被选中
                ...
                if quote_chars and len(quote_chars) == 2:
                    opening_quote, closing_quote = quote_chars
                    selected_pairs[opening_quote] = closing_quote
    ...
    def browse_output_file(self):
        ...
        if filename:
            self.output_filepath_var.set(filename)
    ...
    def test_quote_processing(self):
        ...
        test_window.title("引号处理测试")
        test_window.geometry("500x400")
        
        ttk.Label(test_window, text="输入测试文本:").pack(pady=5)
        input_text = tk.Text(test_window, height=5, wrap=tk.WORD)
        ...
        def process_test():
            ...
            json_output = self.converter.convert_text_to_json_format(
                content, 
                self.narrator_name_var.get(), 
                selected_quote_pairs=selected_pairs
            )
        ...
        result_text = tk.Text(test_window, height=10, wrap=tk.WORD)
    ...
    def start_conversion(self):
        ...
        try:
            ...
            self.log_message("开始读取输入文件...")
            
            with open(input_file, 'r', encoding='utf-8') as f:
                input_text = f.read()
            
            self.progress_var.set(30)
            self.log_message(f"成功读取文件，共{len(input_text)}个字符")
            
            self.status_var.set("正在转换文本...")
            self.progress_var.set(50)
            self.log_message(f"开始转换文本... (根据UI选项移除引号)")
            ...
            self.log_message("文件保存成功！")
            
            messagebox.showinfo("成功", "文件转换并保存成功！")
            
        except FileNotFoundError:
            error_msg = f"找不到输入文件: {input_file}"
    ...
    def open_config_manager(self):
        ...
        config_text = tk.Text(config_window, wrap=tk.WORD)
        config_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        current_mapping = self.config_manager.get_character_mapping()
        config_content = ""
        for name, ids in current_mapping.items():
            config_content += f"{name}={','.join(map(str, ids))}\n"
